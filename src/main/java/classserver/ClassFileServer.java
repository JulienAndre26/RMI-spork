 ...
 * doc for the "Main" method for how to run this server.
 */
public class ClassFileServer extends ClassServer {
     ...
     *            the classpath where the server locates classes
     */
    private ClassFileServer(int port, String paths) throws java.io.IOException
    {
        super(port);
        if (paths != null)
    ...
    /**
     * Main method to create the class server that reads class files. This takes two optional
     * command line arguments, the port on which the server accepts requests and the root of the
     * classpath. To start up the server: <br>
     * <br>
     * <p>
     ...
     * <br>
     * <p>
     * The codebase of an RMI server using this webserver would simply contain a URL with the host
     * and port of the web server (if the webserver's classpath is the same as the RMI server's
     * classpath): <br>
     * <br>
     * <p>
     ...
     * <br>
     * <p>
     * You can create your own class server inside your RMI server application instead of running
     * one separately. In your server main simply create a ClassFileServer: <br>
     * <br>
     * <p>
     ...
     * <b>path</b>. The <b>path</b> is a dot separated class name with the ".class" extension
     * removed.
     * 
     * @return the bytecodes for the class
     * @throws ClassNotFoundException
     *             if the class corresponding to <b>path</b> could not be loaded.
     */
    protected byte[] getBytes(String path) throws java.io.IOException, ClassNotFoundException
    {
        ...
        {
            for (File codebase : codebases)
            {
                try
                {
                    if (codebase.isDirectory())
                    {
                        b = getBytesFromDirectory(path, codebase);
                    }
                    else
                    {
                        b = getBytesFromArchive(path, codebase);
                    }
                    if (b != null)
                        return b;
                }
    ...
    private byte[] getBytesFromResource(String path) throws java.io.IOException
    {
        ...
        // System.out.println("ClassFileServer reading: " +
        // filename+"  length="+length+" from classpath");
        if (length == -1)
        {
     ...
     *            the fqn of the class
     * @param archive
     *            the File that must be a jar or zip archive that may contain the class
     * @return the bytecodes for the class
     * @throws java.io.IOException
     *             if the class cannot be read
     */
            ...
            throws java.io.IOException
    {
        ...
        // System.out.println("ClassFileServer reading: " +
        // filename+"  length="+length+" from jar/xip file "+archive.getAbsolutePath());
        if (length == -1)
        {
            throw new java.io.IOException("File length is unknown: " + filename);
        }
     ...
     *            the fqn of the class
     * @param directory
     *            the File that must be a directory that may contain the class
     * @return the bytecodes for the class
     * @throws java.io.IOException
     *             if the class cannot be read
     */
    private byte[] getBytesFromDirectory(String path, java.io.File directory)
            throws java.io.IOException
    {
        java.io.File f = new java.io.File(directory, path.replace('.', java.io.File.separatorChar)
                + ".class");
        if (!f.exists())
            return null;
    ...
    /**
     * Returns an array of bytes containing the bytecodes for the class represented by the
     * InputStream
     * 
     * @param in
    ...
    {
        System.out
                .println("To use this ClassFileServer set the property -Djava.rmi.server.codebase=http://"
                        + hostname + ":" + port + "/");
        if (codebases == null)
        {
        ...
        {
            System.out
                    .println(" --> This ClassFileServer is reading resources from the following paths");
